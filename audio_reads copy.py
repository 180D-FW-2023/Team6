...
while True:
    try:
        ...
        for idx, value in enumerate(output_data[0]):
            if value > threshold:
                notes_tflite.append(class_labels[idx])

        # Print the classes
        #print("Classes whose values are above", threshold)
        #print("tflite:" + str(notes_tflite)) #these are notes detected by tflite
        ...
        for pair in note_pairs:
            ...
            if letter1 in notes_hps and letter2 in notes_tflite:
                #print(pair)
                both_notes.append(letter1)
        for nl in notes_librosa:
            if nl in notes_hps:
                if nl in notes_tflite:
